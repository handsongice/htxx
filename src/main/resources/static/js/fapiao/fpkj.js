//开启金税盘开票
function openJinShuiPan(type,main,del,gfmcNow,gfshNow,gfdzdhNow,gfyhzhNow,bz){
	if(openCard()){
        ...
        var fphm=pad(result.infoNumber,8);
        alert("请确认现在要开具的发票代码:"+result.infoTypeCode +"\n"+
			"发票号码:"+fphm);
        //开纸票之前，先保存每行商品的税收分类编码
        //changetoxml(main,del);
        //发票数量大于0 ，可以开票
        return invoice(main,del,type,gfmcNow,gfshNow,gfdzdhNow,gfyhzhNow,bz);
...
//金税盘开启检测
 function checkNew(){
	...
	//关闭金税盘
		...
		//开发票
		var main=null;
		function invoice(main,del,type,gfmcNow,gfshNow,gfdzdhNow,gfyhzhNow,bz){
                console.log("invoice:......");
            	console.log("main:"+main);
				//main=eval('(' + main + ')');
            	console.log(main['XFYHZH']);
            	//console.log(main);
