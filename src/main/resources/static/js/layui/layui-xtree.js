...
layuiXtree.prototype.Rendering = function() {
	...
	var xtree_items = $('.layui-xtree-item');
	var xtree_icons = $('.layui-xtree-icon');
	var xtree_nullicons = $('.layui-xtree-icon-null');
	...
	_this._form.on('checkbox(xtreeck' + _this._containerid + ')', function(da) {
		...
		var xtree_chis = getClass(da.elem.parentNode, 'layui-xtree-item');
	...
	if(_this._ckall) {
		_this._form.on('checkbox(xtreeckall' + _this._containerid + ')', function(data) {
			var xtree_allck = getClass(data.elem.parentNode.parentNode, 'layui-form-checkbox');
			for(var i = 0; i < xtree_allck.length; i++) {
				if(xtree_allck[i].getAttribute('class').indexOf('layui-checkbox-disbaled') == -1) {
					...
					getClass(xtree_allck[i].parentNode, 'layui-xtree-checkbox')[0].checked = data.elem.checked;
				}
			}
			_this._ckallSuccess();
		});
