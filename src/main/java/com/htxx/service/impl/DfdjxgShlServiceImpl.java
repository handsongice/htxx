...
import com.htxx.mapper.DfdjxgShlMapper;
...
@Service
@Transactional
public class DfdjxgShlServiceImpl implements DfdjxgShlService {
    @Autowired
    DfdjxgShlMapper dfzypShlMapper;

    @Override
    public List<DfdjxgShl> getDfdjxgShlList(Map<String, Object> params) throws Exception {
        List<DfdjxgShl> dfzypShls = dfzypShlMapper.selectByParams(params);
        return dfzypShls;
    }

    @Override
    public ResultMap addDfdjxgShl(DfdjxgShl dfzypShl) throws Exception {
        ...
        if("line".equals(dfzypShl.getStyle())) {
            //更新父节点
            DfdjxgShl _fpsgShl1 = new DfdjxgShl();
            String[] froms = null;
            String[] tos = null;
            Long _pid = null;
            Long _id = null;
    ...
    @Override
    public ResultMap updateDfdjxgShl(DfdjxgShl dfzypShl) throws Exception {
        SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        //设置创建时间
    ...
    @Override
    public DfdjxgShl getDfdjxgShlById(Long id) {
        return dfzypShlMapper.selectByPrimaryKey(id);
    }

    @Override
    public ResultMap delDfdjxgShl(Long id) {
        DfdjxgShl dfzypShl = dfzypShlMapper.selectByPrimaryKey(id);
        if(null == dfzypShl) {
        ...
        if("node".equals(dfzypShl.getStyle())) {
            ...
            if(null != dfzypShl.getParentId() && dfzypShl.getParentId() > 0) {
                //更新父节点
                DfdjxgShl ufpsgShl1 = new DfdjxgShl();
